class Refeicao < ActiveRecord::Base
  ...
  class << self
    ...
    def by_usuario_date(date,usuario)
      default_select.where("extract(day from data) = ? AND extract(month from data) = ? AND extract(year from data) = ? AND usuario_id=?", date.day, date.month, date.year,usuario.id).includes(:usuario,:tipo_refeicao,{:refeicao_alimentos=>{:alimento=>{:componente_alimentos=>:componente}}},{:refeicao_alimentos=>{:alimento=>{:grupo_alimentos=>:grupo}}}) 
    end
    def default_select
